...
import com.example.hospitalregistration.adapter.WchDoctorAdapter
...
class WhmDoctorListActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        WhmRetrofitClient.api.doctors(departmentId).enqueue(object : Callback<List<Doctor>> {
            override fun onResponse(call: Call<List<Doctor>>, response: Response<List<Doctor>>) {
                val list = response.body() ?: ArrayList()
                recyclerView.adapter = WchDoctorAdapter(list) { doctor ->
                    val intent = Intent(this@WhmDoctorListActivity, WhmDoctorDetailActivity::class.java)
            ...
            override fun onFailure(call: Call<List<Doctor>>, t: Throwable) {
                Toast.makeText(this@WhmDoctorListActivity, "医生加载失败", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
